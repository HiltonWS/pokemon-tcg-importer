...
def __find_file_in_google_drive__():
    if FOLDER_DATABASE_ID:
        query = f"name = '{os.path.basename(DB_PATH)}' and trashed = false and '{FOLDER_DATABASE_ID}' in parents"
    results = __DRIVE__.files().list(q=query, spaces='drive', fields='files(id, name)').execute()
...
def upload_to_google_drive():
    if __FILE_ID__:
        ...
        updated_file = __DRIVE__.files().update(fileId= __FILE_ID__, media_body=media).execute()
        ...
        media = MediaFileUpload(DB_PATH, resumable=True)
        __DRIVE__.files().create(body=file_metadata, media_body=media, fields='id').execute()
